...
Для MVP recording service/repository можно не создавать. Нужно только не смешивать S3 upload с `MediasoupService`, чтобы запись можно было добавить отдельным worker без переписывания signaling flow.
...
- Mantine UI используется как базовая component library для frontend service (`conference-web`; в текущем workspace это может быть `apps/web`).
